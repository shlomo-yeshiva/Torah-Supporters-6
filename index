...
<body>
    ...
    <script>
        ...
        const NEDARIM_CONFIG = {
            ...
            // 4. לחלופין, פתח https://www.matara.pro/nedarimplus/online/?mosad=1000642 ובדוק בקוד המקור
            apiPassword: 'ep348', // ⚠️ לא משמש כשמשתמשים בפרוקסי - נשאר לתאימות
            ...
            matchPlusApiUrl: 'https://www.matara.pro/nedarimplus/V6/MatchPlus.aspx', // API MatchPlus
            // ⚠️ הגדר כאן את ה-URL של השרת פרוקסי (Google Apps Script)
            // הוראות: https://script.google.com - צור פרויקט חדש, העתק את הקוד מ-NedarimProxy.gs, פרסם כ-Web App
            proxyUrl: 'https://script.google.com/macros/s/AKfycbwQs0FeHwstYtvYtD3wqr0ggTvZIsSCqv6K7Rdz90QnrCc5wKzAMMSrEVDsvVyH0FbY/exec', // שרת פרוקסי - Google Apps Script
            useProxy: true, // ✅ משתמש בפרוקסי להסתרת API password
            ...
            totalDonatedFromAPI: null, // הסכום הכולל מ-ShowGoal API (null = עדיין לא נטען)
            goalFromAPI: null, // היעד מ-ShowGoal API (null = עדיין לא נטען)
            apiDataLoaded: false // האם הנתונים נטענו מ-API
        };
        ...
        function restartDonorsAutoScroll() {
            ...
            function step(timestamp) {
                ...
                // הגלילה ממשיכה בלולאה אינסופית - לא עוצרת ולא מציגה מסך ריק
                // הקריאה לנתונים מתבצעת מאחורי הקלעים כל 5 דקות
        ...
        async function uploadOfflineDonation(matrimName, clientName, amount, comments = '', groupName = '', matrimId = null) {
            try {
                ...
                console.log('📦 תגובה מנדרים פלוס (העלאה):', {
                ...
                // אם קוד התגובה הוא 200 (OK), זה נחשב להצלחה גם אם אין JSON
                if (response.ok && response.status === 200) {
                    ...
                    try {
                        ...
                        if (data.Status === 'OK' || data.success === true) {
                            console.log('✅ העלאה הצליחה (JSON עם Status OK):', data);
                        ...
                        console.log('✅ העלאה הצליחה (JSON ללא Status מוגדר):', data);
                        ...
                        if (responseLower.includes('ok') || 
                            ...
                            console.log('✅ העלאה הצליחה (תגובה טקסטואלית):', responseText.substring(0, 100));
                            return { success: true, message: 'התרומה הועלתה בהצלחה' };
                        }
                        
                        // אם יש תגובה ארוכה שלא JSON ולא מכילה הצלחה, זה כנראה שגיאה
                        console.warn('⚠️ תגובה לא צפויה מהשרת:', responseText.substring(0, 200));
                        // אבל עדיין מחזירים הצלחה כי קוד 200 = הצלחה
                        return { success: true, message: 'התרומה הועלתה בהצלחה (תגובה לא סטנדרטית מהשרת)' };
                    }
                } else {
        ...
        async function fetchMatrimFromMatchPlusWithSearch(searchTerm = '') {
            try {
                ...
                // בודק אם יש הגבלת פניות
                if (data && typeof data === 'object' && !Array.isArray(data)) {
                    const dataString = JSON.stringify(data).toLowerCase();
                    if (dataString.includes('500') && (dataString.includes('פניות') || dataString.includes('מוגבלת') || dataString.includes('לא ניתן להציג'))) {
                        throw new Error('הגבלת 500 פניות לשעה - נסה שוב מאוחר יותר');
                    }
                }
                
                // בודק אם התגובה היא מחרוזת עם הודעת שגיאה
                if (typeof data === 'string') {
                    const dataLower = data.toLowerCase();
                    if (dataLower.includes('500') && (dataLower.includes('פניות') || dataLower.includes('מוגבלת') || dataLower.includes('לא ניתן להציג'))) {
                        throw new Error('הגבלת 500 פניות לשעה - נסה שוב מאוחר יותר');
                    }
                }
            ...
            try {
                ...
                for (let i = 0; i < hebrewLetters.length; i++) {
                    ...
                    try {
                        ...
                        // המתנה קטנה בין קריאות כדי לא להעמיס על השרת (הגדלנו ל-500ms כדי להימנע מהגבלת 500 פניות)
                        await new Promise(resolve => setTimeout(resolve, 500));
                    } catch (error) {
                        console.warn(`שגיאה בחיפוש לפי "${letter}":`, error);
                        // אם זו הגבלת פניות, נעצור את כל החיפושים
                        if (error.message && error.message.includes('500 פניות')) {
                            console.error('⚠️ הגבלת 500 פניות לשעה - עוצר את החיפושים');
                            throw error;
                        }
                        // ממשיך עם האות הבאה
                    }
        ...
        // בודקת מספר אפשרויות: ShowGoal API וגם getCampaignData API
        // תיעוד רשמי: https://matara.pro/nedarimplus/ApiDocumentation.html
        // ⚠️ חשוב: יש לבדוק את התיעוד הרשמי כדי לוודא שהפניות והשדות תואמים
        // דף אונליין: https://www.matara.pro/nedarimplus/online/?mosad=1000642
        async function fetchTotalFromMatchPlus() {
            // מאפס את המשתנים לפני חיפוש חדש
            let foundAmount = null;
            let foundGoal = null;
            
            // אם משתמשים בפרוקסי, ננסה דרך הפרוקסי קודם (להסתרת API password)
            if (NEDARIM_CONFIG.useProxy && NEDARIM_CONFIG.proxyUrl) {
                try {
                    console.log('═══════════════════════════════════════════════════════');
                    console.log('🔐 משתמש בשרת פרוקסי (API password מוסתר)');
                    const proxyUrl = `${NEDARIM_CONFIG.proxyUrl}?action=ShowGoal&matchingId=${NEDARIM_CONFIG.matchingId}`;
                    console.log('🔗 פרוקסי URL:', proxyUrl);
                    
                    const response = await fetch(proxyUrl);
                    console.log('📡 תגובת הפרוקסי:', response.status, response.statusText);
                    
                    if (response.ok) {
                        const data = await response.json();
                        console.log('📦 נתונים מהפרוקסי:', JSON.stringify(data, null, 2));
                        
                        // מחפש את השדות Donated ו-Goal
                        if (data && typeof data === 'object' && !Array.isArray(data)) {
                            if (data.Donated !== undefined && data.Donated !== null) {
                                foundAmount = parseNumericValue(data.Donated);
                                console.log(`✅ מצאנו סכום דרך פרוקסי: ${foundAmount?.toLocaleString()} ש"ח`);
                            }
                            if (data.Goal !== undefined && data.Goal !== null) {
                                foundGoal = parseNumericValue(data.Goal);
                                console.log(`✅ מצאנו יעד דרך פרוקסי: ${foundGoal?.toLocaleString()} ש"ח`);
                            }
                            
                            // אם לא מצאנו בשדות הספציפיים, נחפש בכל השדות
                            if (foundAmount === null) {
                                for (const key in data) {
                                    const value = parseNumericValue(data[key]);
                                    if (value !== null && value > 100000) {
                                        const keyLower = key.toLowerCase();
                                        if (keyLower.includes('donat') || keyLower.includes('total') || keyLower.includes('amount') || keyLower.includes('collected')) {
                                            foundAmount = value;
                                            console.log(`✅ מצאנו סכום דרך פרוקסי: ${key} = ${value.toLocaleString()} ש"ח`);
                                            break;
                                        }
                                    }
                                }
                            ...
                            if (foundAmount !== null && foundAmount > 0) {
                                ...
                                NEDARIM_CONFIG.goalFromAPI = foundGoal || 0;
                                NEDARIM_CONFIG.apiDataLoaded = true;
                                console.log(`✅ ✅ ✅ מצאנו את הסכום דרך פרוקסי! יעד=${NEDARIM_CONFIG.goalFromAPI.toLocaleString()}, נאסף=${NEDARIM_CONFIG.totalDonatedFromAPI.toLocaleString()}`);
                                return { Goal: NEDARIM_CONFIG.goalFromAPI, Donated: NEDARIM_CONFIG.totalDonatedFromAPI };
                            }
                        }
                    }
                } catch (proxyError) {
                    console.warn('⚠️ פרוקסי נכשל, מנסה ישירות:', proxyError.message);
                    // נמשיך לניסיונות הרגילים אם הפרוקסי נכשל
                }
            }
            
            try {
                // ניסיון 1: ShowGoal API (הקיים)
                try {
                    const url1 = `${NEDARIM_CONFIG.matchPlusApiUrl}?Action=ShowGoal&MatchingId=${NEDARIM_CONFIG.matchingId}`;
                    console.log('═══════════════════════════════════════════════════════');
                    console.log('🔍 ניסיון 1: מושך סכום כולל מ-ShowGoal');
                    console.log('🔗 URL:', url1);
                    
                    const response1 = await fetch(url1);
                    console.log('📡 תגובת ה-API (ניסיון 1):', response1.status, response1.statusText);
                    console.log('📡 Headers:', Object.fromEntries(response1.headers.entries()));
                    if (response1.ok) {
                        const data1 = await response1.json();
                        console.log('📦 נתונים מ-ShowGoal (כל הנתונים):', JSON.stringify(data1, null, 2));
                        console.log('📦 סוג הנתונים:', typeof data1, Array.isArray(data1) ? '(מערך)' : '(אובייקט)');
                        
                        // בודק את כל השדות האפשריים לסכום
                        const possibleAmountFields = ['Donated', 'totalAmount', 'totalRaised', 'sum', 'current', 'Total', 'TotalAmount', 'TotalRaised', 'amount', 'Amount', 'Collected', 'collected', 'TotalDonated', 'totalDonated'];
                        const possibleGoalFields = ['Goal', 'goal', 'target', 'Target', 'totalGoal', 'TotalGoal'];
                        
                        let foundAmount = null;
                        let foundGoal = null;
                        
                        // מחפש את השדה הנכון לסכום
                        for (const field of possibleAmountFields) {
                            if (data1 && typeof data1[field] !== 'undefined' && data1[field] !== null) {
                                const rawValue = data1[field];
                                const value = parseNumericValue(rawValue);
                                console.log(`🔍 בודק שדה ${field}: ערך גולמי="${rawValue}", ערך מפורס=${value}`);
                                if (value !== null && (value > 0 || foundAmount === null)) {
                                    foundAmount = value;
                                    console.log(`✅ מצאנו שדה סכום: ${field} = ${value.toLocaleString()} ש"ח`);
                                }
                            }
                        }
                        
                        // מחפש את השדה הנכון ליעד
                        for (const field of possibleGoalFields) {
                            if (data1 && typeof data1[field] !== 'undefined' && data1[field] !== null) {
                                const rawValue = data1[field];
                                const value = parseNumericValue(rawValue);
                                if (value !== null && (value > 0 || foundGoal === null)) {
                                    foundGoal = value;
                                    console.log(`✅ מצאנו שדה יעד: ${field} = ${value.toLocaleString()}`);
                                }
                            }
                        }
                        
                        // אם לא מצאנו בשדות הספציפיים, נבדוק את כל השדות באובייקט
                        if (foundAmount === null && data1 && typeof data1 === 'object' && !Array.isArray(data1)) {
                            console.log('🔍 לא מצאנו בשדות הספציפיים, בודק את כל השדות באובייקט...');
                            for (const key in data1) {
                                const rawValue = data1[key];
                                // בודק אם זה נראה כמו סכום (מספר או מחרוזת מספרית)
                                if (rawValue !== null && rawValue !== undefined) {
                                    const value = parseNumericValue(rawValue);
                                    if (value !== null && value > 1000) { // סכום סביר (יותר מ-1000 ש"ח)
                                        console.log(`🔍 מצאנו שדה פוטנציאלי לסכום: ${key} = ${rawValue} (מפורס: ${value.toLocaleString()})`);
                                        if (foundAmount === null || value > foundAmount) {
                                            foundAmount = value;
                                            console.log(`✅ עדכנו שדה סכום: ${key} = ${value.toLocaleString()} ש"ח`);
                                        }
                                    }
                                }
                            }
                    ...
                    const totalFromDonors = donors.reduce((sum, d) => sum + (d.amount || 0), 0);
                            ...
                            // אם ה-API מחזיר 0 אבל יש סכום מהתרימים, זה אומר שה-API לא עובד נכון
                            // במקרה הזה, נשתמש בסכום מהתרימים
                            if (foundAmount === 0 && totalFromDonors > 0) {
                                console.warn('⚠️ ה-API מחזיר 0 אבל יש סכום מהתרימים - כנראה ה-API לא עובד נכון');
                                console.warn(`   נשתמש בסכום מהתרימים: ${totalFromDonors.toLocaleString()} ש"ח`);
                                // לא נשתמש ב-API במקרה הזה
                                return null;
                            }
                            ...
                            NEDARIM_CONFIG.goalFromAPI = foundGoal || 0;
                            NEDARIM_CONFIG.apiDataLoaded = true;
                            console.log(`✅ עדכנו מ-ShowGoal: יעד=${NEDARIM_CONFIG.goalFromAPI.toLocaleString()}, נאסף=${NEDARIM_CONFIG.totalDonatedFromAPI.toLocaleString()}`);
                            
                    const difference = NEDARIM_CONFIG.totalDonatedFromAPI - totalFromDonors;
                            if (Math.abs(difference) > 0) {
                                console.log(`ℹ️ יש הבדל של ${difference.toLocaleString()} ש"ח בין הסכום הכולל לסכום המתרימים`);
                                console.log(`   סכום כולל מ-API: ${NEDARIM_CONFIG.totalDonatedFromAPI.toLocaleString()} ש"ח`);
                        ...
                        console.log(`   ההפרש כנראה מתרומות כלליות שלא קשורות למתרימים ספציפיים`);
                                    ...
                                    console.log(`   הסכום מהתרימים גבוה יותר - ייתכן שיש תרומות מקומיות שעדיין לא הועלו לנדרים פלוס`);
                                }
                    }
                    ...
                    console.error('❌ ShowGoal API נכשל:', error1);
                    console.error('❌ פרטי השגיאה:', error1.message);
                    console.error('❌ סוג השגיאה:', error1.name);
                    if (error1.stack) {
                        console.error('❌ Stack trace:', error1.stack);
                    }
                }
                
                // ניסיון 2: getCampaignData API (המוצע)
                try {
                    const url2 = `https://matara.pro/nedarimplus/v6/api/getCampaignData?mosad=${NEDARIM_CONFIG.mosadId}&password=${NEDARIM_CONFIG.apiPassword}`;
                    console.log('═══════════════════════════════════════════════════════');
                    console.log('🔍 ניסיון 2: מושך סכום כולל מ-getCampaignData');
                    console.log('🔗 URL:', url2);
                    
                    const response2 = await fetch(url2);
                    console.log('📡 תגובת ה-API (ניסיון 2):', response2.status, response2.statusText);
                    if (response2.ok) {
                        const data2 = await response2.json();
                        console.log('📦 נתונים מ-getCampaignData (כל הנתונים):', JSON.stringify(data2, null, 2));
                        console.log('📦 סוג הנתונים:', typeof data2, Array.isArray(data2) ? '(מערך)' : '(אובייקט)');
                        
                        // בודק את כל השדות האפשריים
                        const possibleAmountFields = ['Donated', 'totalAmount', 'totalRaised', 'sum', 'current', 'Total', 'TotalAmount', 'TotalRaised', 'amount', 'Amount', 'collected', 'Collected', 'TotalDonated', 'totalDonated'];
                        const possibleGoalFields = ['Goal', 'goal', 'target', 'Target', 'totalGoal', 'TotalGoal'];
                        ...
                        for (const field of possibleAmountFields) {
                            if (data2 && typeof data2[field] !== 'undefined' && data2[field] !== null) {
                                const rawValue = data2[field];
                                const value = parseNumericValue(rawValue);
                                console.log(`🔍 בודק שדה ${field}: ערך גולמי="${rawValue}", ערך מפורס=${value}`);
                                if (value !== null && (value > 0 || foundAmount === null)) {
                                    foundAmount = value;
                        ...
                        // אם לא מצאנו בשדות הספציפיים, נבדוק את כל השדות באובייקט
                        if (foundAmount === null && data2 && typeof data2 === 'object' && !Array.isArray(data2)) {
                            console.log('🔍 לא מצאנו בשדות הספציפיים, בודק את כל השדות באובייקט...');
                            for (const key in data2) {
                                const rawValue = data2[key];
                                if (rawValue !== null && rawValue !== undefined) {
                                    const value = parseNumericValue(rawValue);
                                    if (value !== null && value > 1000) { // סכום סביר
                                        console.log(`🔍 מצאנו שדה פוטנציאלי לסכום: ${key} = ${rawValue} (מפורס: ${value.toLocaleString()})`);
                                        if (foundAmount === null || value > foundAmount) {
                                            foundAmount = value;
                                            console.log(`✅ עדכנו שדה סכום: ${key} = ${value.toLocaleString()} ש"ח`);
                        ...
                        if (foundAmount !== null && foundAmount > 0) {
                            ...
                            NEDARIM_CONFIG.goalFromAPI = foundGoal || 0;
                            NEDARIM_CONFIG.apiDataLoaded = true;
                            console.log(`✅ עדכנו מ-getCampaignData: יעד=${NEDARIM_CONFIG.goalFromAPI.toLocaleString()}, נאסף=${NEDARIM_CONFIG.totalDonatedFromAPI.toLocaleString()}`);
                    ...
                    console.error('❌ getCampaignData API נכשל:', error2);
                    console.error('❌ פרטי השגיאה:', error2.message);
                    console.error('❌ סוג השגיאה:', error2.name);
                }
                
                // ניסיון 3: GetMosad API (דרך דף האונליין) - זהו ה-API המחובר ישירות לדף האונליין
                try {
                    const url3 = `${NEDARIM_CONFIG.onlineApiUrl}?Action=GetMosad&MosadId=${NEDARIM_CONFIG.mosadId}&S=&_=${Date.now()}`;
                    console.log('═══════════════════════════════════════════════════════');
                    console.log('🔍 ניסיון 3: מושך סכום כולל מ-GetMosad (מחובר לדף האונליין)');
                    console.log('🔗 URL:', url3);
                    console.log('🔗 דף אונליין: https://www.matara.pro/nedarimplus/online/?mosad=' + NEDARIM_CONFIG.mosadId);
                    ...
                    console.log('📡 תגובת ה-API (ניסיון 3):', response3.status, response3.statusText);
                    
                    if (response3.ok) {
                        const responseText = await response3.text();
                        console.log('📄 תגובה גולמית (ניסיון 3, 500 תווים ראשונים):', responseText.substring(0, 500));
                        
                        let data3;
                        try {
                            data3 = JSON.parse(responseText);
                        } catch (parseError) {
                            console.error('❌ שגיאה בפרסור JSON (ניסיון 3):', parseError);
                            console.error('❌ התגובה לא בפורמט JSON');
                            throw new Error('התגובה לא בפורמט JSON');
                        }
                        console.log('📦 נתונים מ-GetMosad (כל הנתונים):', JSON.stringify(data3, null, 2));
                        console.log('📦 סוג הנתונים:', typeof data3, Array.isArray(data3) ? '(מערך)' : '(אובייקט)');
                        
                        // בודק את כל השדות האפשריים - מורחב עם שדות נוספים שנמצאים בדף האונליין
                        const possibleAmountFields = [
                            'Donated', 'donated', 'DonatedAmount', 'donatedAmount',
                            'totalAmount', 'TotalAmount', 'total_amount', 'Total_Amount',
                            'totalRaised', 'TotalRaised', 'total_raised', 'Total_Raised',
                            'sum', 'Sum', 'TotalSum', 'totalSum',
                            'current', 'Current', 'CurrentAmount', 'currentAmount',
                            'Total', 'total', 'Amount', 'amount',
                            'collected', 'Collected', 'TotalCollected', 'totalCollected',
                            'TotalDonated', 'totalDonated', 'total_donated', 'Total_Donated',
                            'Raised', 'raised', 'TotalRaisedAmount', 'totalRaisedAmount'
                        ];
                        const possibleGoalFields = ['Goal', 'goal', 'target', 'Target', 'totalGoal', 'TotalGoal', 'TargetAmount', 'targetAmount'];
                        
                        let foundAmount = null;
                        let foundGoal = null;
                        
                        // בודק גם שדות מקוננים (nested)
                        const checkNestedFields = (obj, prefix = '') => {
                            ...
                            for (const key in obj) {
                                ...
                                // בודק אם זה שדה סכום
                                if (possibleAmountFields.includes(key)) {
                                    const numValue = parseNumericValue(rawValue);
                                    if (numValue !== null && (numValue > 0 || foundAmount === null)) {
                                        foundAmount = numValue;
                                        console.log(`✅ מצאנו שדה סכום: ${fullKey} = ${rawValue} (מפורס: ${numValue.toLocaleString()})`);
                                    }
                                }
                                
                                // בודק אם זה שדה יעד
                                if (possibleGoalFields.includes(key)) {
                                    const numValue = parseNumericValue(rawValue);
                                    if (numValue !== null && (numValue > 0 || foundGoal === null)) {
                                        foundGoal = numValue;
                                        console.log(`✅ מצאנו שדה יעד: ${fullKey} = ${rawValue} (מפורס: ${numValue.toLocaleString()})`);
                                    }
                                }
                                
                                // בודק גם שדות שלא ברשימה אבל נראים כמו סכומים (מספרים גדולים)
                                if (typeof rawValue !== 'object' && rawValue !== null) {
                                    const numValue = parseNumericValue(rawValue);
                                    if (numValue !== null && numValue > 1000) { // סכום סביר
                                        // בודק אם השם של השדה מכיל מילות מפתח הקשורות לסכום
                                        const keyLower = key.toLowerCase();
                                        if ((keyLower.includes('donat') || keyLower.includes('total') || keyLower.includes('amount') || keyLower.includes('collected') || keyLower.includes('sum')) && (foundAmount === null || numValue > foundAmount)) {
                                            foundAmount = numValue;
                                            console.log(`✅ מצאנו שדה סכום (לפי שם השדה): ${fullKey} = ${rawValue} (מפורס: ${numValue.toLocaleString()})`);
                                        }
                                    }
                                }
                                ...
                                // בודק מקונן
                                if (typeof rawValue === 'object' && rawValue !== null) {
                                    checkNestedFields(rawValue, fullKey);
                                }
                            }
                        };
                        
                        checkNestedFields(data3);
                        ...
                        if (foundAmount !== null && foundAmount > 0) {
                            ...
                            NEDARIM_CONFIG.goalFromAPI = foundGoal || 0;
                            NEDARIM_CONFIG.apiDataLoaded = true;
                            console.log(`✅ עדכנו מ-GetMosad: יעד=${NEDARIM_CONFIG.goalFromAPI.toLocaleString()}, נאסף=${NEDARIM_CONFIG.totalDonatedFromAPI.toLocaleString()}`);
                    ...
                    console.error('❌ GetMosad API נכשל:', error3);
                    console.error('❌ פרטי השגיאה:', error3.message);
                    console.error('❌ סוג השגיאה:', error3.name);
                }
                
                // ניסיון 4: ניסיון נוסף עם GetMosad עם פרמטרים שונים
                try {
                    const url4 = `${NEDARIM_CONFIG.onlineApiUrl}?Action=GetMosad&MosadId=${NEDARIM_CONFIG.mosadId}`;
                    console.log('🔍 ניסיון 4: ניסיון נוסף עם GetMosad (ללא פרמטר S):', url4);
                    ...
                    if (response4.ok) {
                        ...
                        console.log('📦 נתונים מ-GetMosad (ניסיון 4 - כל הנתונים):', JSON.stringify(data4, null, 2));
                        
                        // ⚠️ חשוב: GetMosad מחזיר את ה-Matching ID האמיתי!
                        // אם יש Matching ID, ננסה למשוך את הסכום מה-ShowGoal API עם ה-Matching ID הנכון
                        let matchingIdFromMosad = null;
                        if (data4 && data4.Matching) {
                            const matchingParts = data4.Matching.split(':');
                            if (matchingParts.length >= 2) {
                                matchingIdFromMosad = matchingParts[1]; // 715 במקרה הזה
                                console.log(`🔍 מצאנו Matching ID בנתוני המוסד: ${matchingIdFromMosad}`);
                                console.log(`💡 זה שונה מה-Matching ID שהוגדר בקוד: ${NEDARIM_CONFIG.matchingId}`);
                                console.log(`💡 ננסה למשוך את הסכום מה-ShowGoal API עם MatchingId=${matchingIdFromMosad}`);
                                
                                // ניסיון נוסף עם ה-Matching ID הנכון!
                                try {
                                    const url5 = `${NEDARIM_CONFIG.matchPlusApiUrl}?Action=ShowGoal&MatchingId=${matchingIdFromMosad}`;
                                    console.log('═══════════════════════════════════════════════════════');
                                    console.log('🔍 ניסיון 5: מושך סכום כולל מ-ShowGoal עם Matching ID מהמוסד');
                                    console.log('🔗 URL:', url5);
                                    
                                    const response5 = await fetch(url5);
                                    console.log('📡 תגובת ה-API (ניסיון 5):', response5.status, response5.statusText);
                                    
                                    if (response5.ok) {
                                        const data5 = await response5.json();
                                        console.log('📦 נתונים מ-ShowGoal (ניסיון 5 - כל הנתונים):', JSON.stringify(data5, null, 2));
                                        
                                        // מחפש את השדה Donated
                                        if (data5 && typeof data5 === 'object' && !Array.isArray(data5)) {
                                            for (const key in data5) {
                                                const rawValue = data5[key];
                                                const value = parseNumericValue(rawValue);
                                                
                                                if (value !== null && value > 100000) { // סכום גדול סביר
                                                    const keyLower = key.toLowerCase();
                                                    if ((keyLower.includes('donat') || keyLower.includes('total') || keyLower.includes('amount') || keyLower.includes('collected') || keyLower.includes('sum') || keyLower.includes('raised'))) {
                                                        foundAmount = value;
                                                        console.log(`✅ מצאנו סכום! ${key} = ${rawValue} (מפורס: ${value.toLocaleString()} ש"ח)`);
                                                    }
                                                    
                                                    if (keyLower.includes('goal') || keyLower.includes('target')) {
                                                        foundGoal = value;
                                                        console.log(`✅ מצאנו יעד! ${key} = ${rawValue} (מפורס: ${value.toLocaleString()} ש"ח)`);
                                                    }
                                                }
                                            }
                                            ...
                                            if (foundAmount !== null && foundAmount > 0) {
                                                ...
                                                NEDARIM_CONFIG.goalFromAPI = foundGoal || 0;
                                                NEDARIM_CONFIG.apiDataLoaded = true;
                                                console.log(`✅ ✅ ✅ מצאנו את הסכום! יעד=${NEDARIM_CONFIG.goalFromAPI.toLocaleString()}, נאסף=${NEDARIM_CONFIG.totalDonatedFromAPI.toLocaleString()}`);
                                                
                                                const difference = NEDARIM_CONFIG.totalDonatedFromAPI - totalFromDonors;
                                                console.log(`📊 סכום מדף אונליין (צפוי): 510,713 ש"ח`);
                                                console.log(`📊 סכום מ-API (מצאנו): ${NEDARIM_CONFIG.totalDonatedFromAPI.toLocaleString()} ש"ח`);
                                                console.log(`📊 ההפרש: ${(510713 - NEDARIM_CONFIG.totalDonatedFromAPI).toLocaleString()} ש"ח`);
                                    ...
                                    console.warn('⚠️ ניסיון 5 (ShowGoal עם Matching ID מהמוסד) נכשל:', error5.message);
                                }
                            }
                        }
                        
                        let foundAmount4 = null;
                        let foundGoal4 = null;
                        
                        // בודק את כל השדות באובייקט (גם מקוננים)
                        const checkAllFields = (obj, prefix = '') => {
                            ...
                            for (const key in obj) {
                                ...
                                if (value !== null && value > 100000) { // סכום גדול סביר (יותר מ-100,000 ש"ח)
                                    const keyLower = key.toLowerCase();
                                    // בודק אם השם של השדה נראה כמו סכום כולל
                                    if (keyLower.includes('donat') || keyLower.includes('total') || keyLower.includes('amount') || keyLower.includes('collected') || keyLower.includes('sum') || keyLower.includes('raised')) {
                                        if (foundAmount4 === null || value > foundAmount4) {
                                            foundAmount4 = value;
                                            console.log(`✅ מצאנו שדה סכום (ניסיון 4): ${fullKey} = ${rawValue} (מפורס: ${value.toLocaleString()})`);
                                        }
                                    }
                                    
                                    // בודק יעד
                                    if (keyLower.includes('goal') || keyLower.includes('target')) {
                                        if (foundGoal4 === null || value > foundGoal4) {
                                            foundGoal4 = value;
                                            console.log(`✅ מצאנו שדה יעד (ניסיון 4): ${fullKey} = ${rawValue} (מפורס: ${value.toLocaleString()})`);
                                        }
                                    }
                                }
                                ...
                                // בודק מקונן
                                if (typeof rawValue === 'object' && rawValue !== null) {
                                    checkAllFields(rawValue, fullKey);
                                }
                            }
                        };
                        
                        checkAllFields(data4);
                        
                        if (foundAmount4 !== null && foundAmount4 > 0) {
                            const totalFromDonors = donors.reduce((sum, d) => sum + (d.amount || 0), 0);
                            NEDARIM_CONFIG.totalDonatedFromAPI = foundAmount4;
                            NEDARIM_CONFIG.goalFromAPI = foundGoal4 || NEDARIM_CONFIG.goalFromAPI || 0;
                            NEDARIM_CONFIG.apiDataLoaded = true;
                            console.log(`✅ עדכנו מ-GetMosad (ניסיון 4 - מדף אונליין): יעד=${NEDARIM_CONFIG.goalFromAPI.toLocaleString()}, נאסף=${NEDARIM_CONFIG.totalDonatedFromAPI.toLocaleString()}`);
                            
                            const difference = NEDARIM_CONFIG.totalDonatedFromAPI - totalFromDonors;
                            if (Math.abs(difference) > 0) {
                                console.log(`ℹ️ יש הבדל של ${difference.toLocaleString()} ש"ח בין הסכום הכולל לסכום המתרימים`);
                                console.log(`   סכום כולל מ-API (מדף אונליין): ${NEDARIM_CONFIG.totalDonatedFromAPI.toLocaleString()} ש"ח`);
                                console.log(`   סכום מהתרימים (מקומי): ${totalFromDonors.toLocaleString()} ש"ח`);
                            }
                    ...
                    console.warn('⚠️ ניסיון 4 (GetMosad נוסף) נכשל:', error4.message);
                }
                
                // אם כל ה-APIs מחזירים 0 או null, נחזיר null
                // המערכת תשתמש בסכום מהתרימים במקרה הזה
                console.log('═══════════════════════════════════════════════════════');
                console.log('⚠️ כל ה-APIs נבדקו ולא נמצא סכום תקין');
                console.log('📊 המערכת תשתמש בסכום מחושב מהתרימים המקומיים');
                console.log('💡 טיפ: פתח את הקונסול (F12) כדי לראות את כל הנתונים שה-API מחזיר');
                console.log('💡 אם אתה רואה נתונים בקונסול אבל הסכום לא נכון, שלח את הלוגים לתמיכה');
                console.log('═══════════════════════════════════════════════════════');
                return null;
                
            } catch (error) {
                console.error('❌ שגיאה כללית במשיכת סכום כולל:', error);
                return null;
            }
        }
        ...
        async function loadMatrimFromNedarim(silent = false) {
            ...
            try {
                ...
                // מחפש קבוצה "נדרים פלוס" - לא יוצר חדשה אם כבר קיימת
                ...
                if (!nedarimGroup) {
                    ...
                    console.log('ℹ️ [טעינת מתרימים] נמצאה קבוצה קיימת "נדרים פלוס"');
                }
        ...
        async function syncWithNedarimPlus(silent = false) {
            ...
            try {
                ...
                if (!nedarimGroup) {
                    ...
                    console.log('ℹ️ [סנכרון] נמצאה קבוצה קיימת "נדרים פלוס"');
                }
                ...
                // מעדכן את התצוגות עם הסכום הנכון
                updateHomeStats();
                updateLeadersList();
                updateLiveTargets();
        ...
        function startAutoSync() {
            ...
            // מפעיל עדכון של הסכום הכולל מ-API כל 30 שניות
        ...
        window.addEventListener('load', () => {
            ...
            // עדכון ראשוני של הסכום הכולל מ-API
            setTimeout(async () => {
        ...
        async function checkTotalAmountFromAPI(silent = false) {
            ...
            console.log('═══════════════════════════════════════════════════════');
            console.log('🔗 דף אונליין: https://www.matara.pro/nedarimplus/online/?mosad=1000642');
            console.log('🔗 מוסד ID: 1000642');
            console.log('═══════════════════════════════════════════════════════');
            } else {
